...
%% Solver
timespan=0:0.01:10;     
IC=[0;0;0;0;0;0;0;0];       %Initial conditionss

vr=1;       % Reference linear velocity
wr=1;       % Reference angular velocitys
...
[~,del_v1, del_v2, I_tilde Tr Tl tR_ddot rhoR_ddot]=cellfun(@(t,S) Robust_controller(t,S,vr,wr,L,Kp,Kd,...
...
tR_ddot=cell2mat(tR_ddot);      %Converting to double form
rhoR_ddot=cell2mat(rhoR_ddot);  %Converting to double form
%% Plots
rhoR_dot=S(:,3);
rhoL_dot=S(:,4);
...
figure('Renderer', 'painters', 'Position', [25 25 00 800])
subplot(2,2,1)  
plot(t,tR_ddot ,'Linewidth',2)
title('$\ddot{\theta_R}$','Interpreter','Latex')
box on
grid on
subplot(2,2,2)
plot(t,rhoR_ddot,'Linewidth',2)
box on
title('$\ddot{\rho_R}$','Interpreter','Latex')
grid on
subplot(2,2,[3,4])
